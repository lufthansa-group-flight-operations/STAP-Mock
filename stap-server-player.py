...
class DataPlayer():
    def __init__(self, filepath, skip_time=None):
        self._subscribers = []
        self._lock = threading.Lock()
        self._filepath = filepath
        self._skip_time = skip_time
        self._engine = threading.Thread(target=self.engine, daemon=True)
        self._engine.start()
    ...
    def stream_xml(self, filepath):
        Logger.info(f'Opening file {filepath} for streaming.')
        print_step = 10 * 1024 * 1024 # 10MB
        with open(filepath, 'rb') if filepath.endswith('.xml') else (gzip.open(filepath, 'r') if filepath.endswith('.gz') else None) as input_stream:
            last_pos = 0
            for event, elem in etree.iterparse(input_stream, events=('start', 'end')):
                curr_pos = input_stream.tell()
                if curr_pos > last_pos + print_step:
                    last_pos = curr_pos
                    Logger.info(f'File position {curr_pos/1048576} MB')
                yield event, elem
    ...
    def process_stream(self, element_stream, skip_time=None):
        regex = re.compile('data,([0-9]*),([0-9]*),([0-9]*),(.*)[\\n\\r]+')
        to_skip = skip_time
        first_rtc = None
        if to_skip is not None:
            Logger.info('Skipping {to_skip} minutes of the playback starts...')
        try:
            for event, elem in element_stream:
                if event == 'end' and elem.tag == 'RawData':
                    results = regex.match(elem.text)

                    if results is None:
                        continue

                    realclock = elem.getparent().attrib['TimeStamp']
                    timestamp = int(results.group(1))
                    line = int(results.group(2))
                    label = int(results.group(3), base=8)
                    value = int(results.group(4), base=16)
                    
                    rtc = parser.isoparse(realclock)
                    if rtc.tzinfo is None:
                        rtc = rtc.replace(tzinfo = datetime.timezone.utc)

                    if to_skip is None:
                        yield { 'rtc': rtc, 'ts': timestamp, 'line': line, 'label': label, 'value': value }
                    else:
                        if first_rtc is None:
                            first_rtc = rtc
                        else:
                            if (rtc - first_rtc).total_seconds() / 60 > to_skip:
                                Logger.info('Skipping of the playback time ready.')
                                to_skip = None                        
    ...
    def engine(self):
        iterator = self.process_stream(self.stream_xml(self._filepath), self._skip_time)

        first_data = next(iterator)
        start_time = time.monotonic()
        start_ts = first_data['ts']
        for a429w in iterator:
            ts = a429w['ts']
            
            to_wait = (ts - start_ts) / 1000 - (time.monotonic() - start_time)
            if to_wait > 0:
                time.sleep(to_wait)
            else:
                pass
...
class STAPHandler(socketserver.BaseRequestHandler):
    ...
    def setup(self):
        self._data_source = self.server.data_source
    ...
    def data_generator(self, client):
        ...
        try:
            q = self._data_source.subscribe()
            
            while self.should_run:
                data = q.get()
                if data is None:
                    Logger.info('Data generator quits - data source closed.')
                    break
                
                params = self.session.get(data['line'])
                if params is None:
                    continue
                    
                # todo: simplification - ignoring the type of the channel
                if data['label'] in params:
                    message = f"data,{data['ts']},{data['line']},{data['label']:o},{data['value']:x}".encode('ascii')
            ...
            Logger.info('Error sending data to the client {}:{}. Interrupting the data delivery thread. {}'.format(self.client_address[0], self.client_address[1], e))
                
        finally:
            self._data_source.unsubscribe(q)
...
class ThreadedTCPServer(socketserver.ThreadingMixIn, socketserver.TCPServer):
    def __init__(self, hostandport, handler, data_source):
...
if __name__ == "__main__":
    ...
    try:
        for argument in sys.argv[1:]:
            key, value = argument.split('=', 1) if '=' in argument else (None, None)
            match key:
                case 'in':
                    input_file = value
                case 'skip':
                    skip_time = int(value)
                case 'host':
                    GLOBAL_CONFIG['host'] = value
                case 'port':
                    GLOBAL_CONFIG['port'] = int(value)
    except:
        Logger.error('Invalid command line arguments')
        Logger.error('required: in=<input file path with recorded data .gz or .xml>')
        Logger.error('optional: skip=<number of minutes to skip in the playback as an integer value>')
        Logger.error('optional: host=<bind address for the STAP server>')
        Logger.error('optional: port=<bind port for the STAP server>')
        sys.exit(1)
    
    if input_file is None:
        Logger.error('Expected input file as an argument (.gz or .xml)')
        sys.exit(1)
    ...
    data_player = DataPlayer(input_file, skip_time)
    server = ThreadedTCPServer((GLOBAL_CONFIG['host'], GLOBAL_CONFIG['port']), STAPHandler, data_player)
